...
#python3 yolo_detect.py --model=110615-2.pt --source=madsentest.mp4
...
while True:
    ...
    m_width = 1280
    m_height = 720
    m_height, m_width, channels = frame.shape
    string_to_print = f"Frame count: {frame_count} "
    string_to_print += f"Width: {m_width} Heigth: {m_height}\n"
    ...
    for i, detection in enumerate(detections):
        ...
        if conf > float(min_thresh):
            ...
            # Calculate YOLO-format normalized box [center_x, center_y, width, height]
            ...
            box_w_norm = box_w / frame_w
            box_h_norm = box_h / frame_h
            ...
            string_to_print = (
                f"Label: {classname} ID: {classidx} Confidence: {conf:.2f} Detection count: {object_count} "
                f"Position: center=({center_x_norm:.4f}, {center_y_norm:.4f}) "
                f"Bounds: xmin={x_min_norm:.4f}, ymin={y_min_norm:.4f}, xmax={(xmax/frame_w):.4f}, ymax={(ymax/frame_h):.4f}\n"
                #f"YOLO Box (normalized): center=({center_x_norm:.4f}, {center_y_norm:.4f}) "
                #f"size=({box_w_norm:.4f}, {box_h_norm:.4f}\n) "
            )
            print(string_to_print)
